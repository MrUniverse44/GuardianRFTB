...
public class ShopMenu {
    ...
    public ShopMenu(RigoxRFTB main) {
        plugin = main;
    ...
    private void createInv() {
        String invName = plugin.getStorage().getControl(RigoxFiles.MENUS).getString("menus.shop.inventoryName");

        if(invName == null) invName = "&8Shop Menu";

        invName = ChatColor.translateAlternateColorCodes('&',invName);

        int rows = getRows(plugin.getStorage().getControl(RigoxFiles.MENUS).getInt("menus.shop.inventoryRows"));
    ...
    private void addOnlyFill(ItemStack itemFill,String fillPath) {
        for(int slot : plugin.getStorage().getControl(RigoxFiles.MENUS).getIntegerList(ShopAction.FILL.getPath() + "." + fillPath + ".list.values")) {
            fills.put(slot,itemFill);
        }
    }
    private void addIgnoreFill(ItemStack itemFill,String fillPath) {
        List<Integer> ignoreSlots = plugin.getStorage().getControl(RigoxFiles.MENUS).getIntegerList(ShopAction.FILL.getPath() + "." + fillPath + ".list.values");
    ...
    private void loadFills() {
        FileConfiguration menu = plugin.getStorage().getControl(RigoxFiles.MENUS);
        for(String fills : plugin.getStorage().getContent(RigoxFiles.MENUS,ShopAction.FILL.getPath(),false)) {
