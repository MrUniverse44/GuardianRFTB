package dev.mruniverse.guardianrftb.core.commands;
...
public class MainCMD implements CommandExecutor {
    ...
    public MainCMD(String cmd, GuardianRFTB main) {
        this.command = cmd;
        this.plugin = main;
    }
    ...
    private boolean hasPermission(CommandSender sender, String permission) {
        if(sender instanceof Player) {
            ...
            if (!player.hasPermission(permission)) {
                try {
                    plugin.getUtils().sendMessage(player, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.others.no-perms").replace("%permission%", permission));
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, String[] args) {
        try {
            if (args.length == 0 || args[0].equalsIgnoreCase("help")) {
                if (hasPermission(sender, "GuardianRFTB.help")) {
                    ...
                    if(sender instanceof Player) {
                        if(sender.hasPermission("GuardianRFTB.admin.help")) {
            ...
            if(args[0].equalsIgnoreCase("admin")) {
                if(args.length == 1 || args[1].equalsIgnoreCase("help")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.help")) {
                ...
                if(args[1].equalsIgnoreCase("create")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.create")) {
                        if (args.length == 2) {
                        ...
                        if (plugin.getGameManager().getGame(args[2]) == null) {
                            if (!plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                ...
                                plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.create").replace("%arena_id%", args[2]));
                                return true;
                            }
                ...
                if(args[1].equalsIgnoreCase("enable")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.enable")) {
                        if (args.length == 2) {
                            plugin.getUtils().sendMessage(sender,"&7Bad Usage");
                            return true;
                        }
                ...
                if(args[1].equalsIgnoreCase("disable")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.enable")) {
                        ...
                        if (plugin.getGameManager().getGame(args[2]) != null) {
                            if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                plugin.getStorage().getControl(GuardianFiles.GAMES).set("games." + args[2] + ".enabled",false);
                ...
                if(args[1].equalsIgnoreCase("modes")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.modes")) {
                        plugin.getUtils().sendMessage(sender,"&8» &6CLASSIC &7- &6INFECTED &7- &6DOUBLE_BEAST");
                    }
                }
                if(args[1].equalsIgnoreCase("list")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.list")) {
                        if(plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games")) {
                            boolean status;
                            for (String gameName : plugin.getStorage().getControl(GuardianFiles.GAMES).getConfigurationSection("games").getKeys(false)) {
                                status = plugin.getStorage().getControl(GuardianFiles.GAMES).getBoolean("games." + gameName + ".enabled");
                ...
                if(args[1].equalsIgnoreCase("delete")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.delete")) {
                        if (args.length == 2) {
                            plugin.getUtils().sendMessage(sender, "&7Bad usage.");
                            return true;
                        }
                        if (plugin.getGameManager().getGame(args[2]) != null || plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                            plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.delete").replace("%arena_id%", args[2]));
                            plugin.getStorage().getControl(GuardianFiles.GAMES).set("games." + args[2], null);
                            return true;
                        }
                ...
                if(args[1].equalsIgnoreCase("addChest")) {
                    if(hasPermission(sender,"GuardianRFTB.admin.addChest")) {
                        ...
                        if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                            if(plugin.getStorage().getControl(GuardianFiles.GAMES).get("games." + args[2] + ".chests") != null) {
                                if(plugin.getStorage().getControl(GuardianFiles.GAMES).getStringList("games." + args[2] + ".chests").contains(args[3])) {
                                    plugin.getUtils().sendMessage(sender,"&cThis chest already exists in game '&e" + args[2] + "&c'");
                                    return true;
                                }
                                List<String> chests = plugin.getStorage().getControl(GuardianFiles.GAMES).getStringList("games." + args[2] + ".chests");
                                chests.add(args[3]);
                                plugin.getUtils().sendMessage(sender,"&aChest &b" + args[3] + " &aadded to game &b" + args[2] + "&a.");
                                plugin.getStorage().getControl(GuardianFiles.GAMES).set("games." + args[2] + ".chests",chests);
                                plugin.getStorage().save(SaveMode.GAMES_FILES);
                                return true;
                            }
                ...
                if(sender instanceof Player) {
                    ...
                    if(args[1].equalsIgnoreCase("setlobby")) {
                        if(hasPermission(sender,"GuardianRFTB.admin.setlobby")) {
                            String location = plugin.getUtils().getStringFromLocation(player.getLocation());
                            plugin.getStorage().getControl(GuardianFiles.SETTINGS).set("settings.lobbyLocation",location);
                    ...
                    if (args[1].equalsIgnoreCase("setWaiting")) {
                        if (hasPermission(sender, "GuardianRFTB.admin.locations")) {
                            if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                plugin.getGameManager().setWaiting(args[2], player.getLocation());
                                plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.setWaiting").replace("%arena_id%", args[2]).replace("%spawnType%","Runner").replace("%location%","X: " + player.getLocation().getX() + " Y: " + player.getLocation().getY() + " Z: " + player.getLocation().getZ()));
                                return true;
                            }
                            plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.arenaError").replace("%arena_id%", args[2]));
                            return true;
                    ...
                    if(args[1].equalsIgnoreCase("addChestLocation")) {
                        if(hasPermission(sender,"GuardianRFTB.admin.addChestLocation")) {
                            ...
                            if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                ...
                                if(plugin.getStorage().getControl(GuardianFiles.GAMES).get(path) != null) {
                                    if(plugin.getStorage().getControl(GuardianFiles.GAMES).getStringList(path).contains(args[3])) {
                                        plugin.getUtils().sendMessage(sender,"&cThis chest location already exists in game '&e" + args[2] + "&c'");
                                        return true;
                                    }
                                    List<String> chests = plugin.getStorage().getControl(GuardianFiles.GAMES).getStringList(path);
                                    chests.add(toAdd);
                                    plugin.getUtils().sendMessage(sender,"&aChest Location added to chest &b" + args[3] + " &ain game&b " + args[2] + "&a.");
                                    plugin.getStorage().getControl(GuardianFiles.GAMES).set(path,chests);
                                    plugin.getStorage().save(SaveMode.GAMES_FILES);
                                    return true;
                                }
                                List<String> chests = new ArrayList<>();
                    ...
                    if (args[1].equalsIgnoreCase("setSelectedBeast")) {
                        if (hasPermission(sender, "GuardianRFTB.admin.locations")) {
                            if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                ...
                                plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.setSpawn").replace("%arena_id%", args[2]).replace("%spawnType%","SelectedBeast").replace("%location%","X: " + player.getLocation().getX() + " Y: " + player.getLocation().getY() + " Z: " + player.getLocation().getZ()));
                                return true;
                            }
                            plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.arenaError").replace("%arena_id%", args[2]));
                            return true;
                    ...
                    if (args[1].equalsIgnoreCase("setBeastSpawn")) {
                        if (hasPermission(sender, "GuardianRFTB.admin.locations")) {
                            if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                ...
                                plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.setSpawn").replace("%arena_id%", args[2]).replace("%spawnType%","Beast").replace("%location%","X: " + player.getLocation().getX() + " Y: " + player.getLocation().getY() + " Z: " + player.getLocation().getZ()));
                                return true;
                            }
                            plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.arenaError").replace("%arena_id%", args[2]));
                            return true;
                    ...
                    if (args[1].equalsIgnoreCase("setRunnerSpawn")) {
                        if (hasPermission(sender, "GuardianRFTB.admin.locations")) {
                            if (plugin.getStorage().getControl(GuardianFiles.GAMES).contains("games." + args[2])) {
                                ...
                                plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.setSpawn").replace("%arena_id%", args[2]).replace("%spawnType%","Runner").replace("%location%","X: " + player.getLocation().getX() + " Y: " + player.getLocation().getY() + " Z: " + player.getLocation().getZ()));
                                return true;
                            }
                            plugin.getUtils().sendMessage(sender, plugin.getStorage().getControl(GuardianFiles.MESSAGES).getString("messages.admin.arenaError").replace("%arena_id%", args[2]));
                            return true;
